...
SELECT * FROM Walmart.clean_dataset1;
...
SELECT COUNT(DISTINCT branch) FROM walmart1;

-- Find the minimum quantity sold
SELECT MIN(quantity) FROM walmart1;
...
FROM (SELECT branch, DAYNAME(STR_TO_DATE(date, '%d/%m/%Y')) AS day_name, COUNT(*) AS no_transactions,
...
WITH revenue_2022 AS (SELECT branch, SUM(total) AS revenue FROM walmart1
